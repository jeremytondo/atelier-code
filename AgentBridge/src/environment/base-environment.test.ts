...
describe("parseShellProbeOutput", () => {
  test("ignores startup chatter outside the sentinel-wrapped environment payload", () => {
    const output = ENCODER.encode(
      [
        "Welcome to zsh!\n",
        "__ATELIERCODE_ENV_BEGIN_4c58d0e1__\0",
        "PATH=/opt/homebrew/bin:/usr/bin:/bin\0",
        "HOME=/Users/tester\0",
        "SHELL=/bin/zsh\0",
        "__ATELIERCODE_ENV_END_4c58d0e1__\0",
        "prompt> ",
      ].join(""),
    );
  ...
  test("fails closed when sentinel markers are missing", () => {
    const output = ENCODER.encode("PATH=/usr/bin:/bin\0HOME=/Users/tester\0");

    expect(() => parseShellProbeOutput(output)).toThrow(
      "expected sentinel markers",
    );
  });
});
